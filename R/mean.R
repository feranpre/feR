...
medias <- function(...,by=NULL,decimals=2,
  ...
  if (!exists("result.temp")) return(NA)
    ...
    if(show.interpretation) attr(result.temp, "INTERPRETATION") <- .help.desc.mean(result.temp, lang = lang, code = "HELP")
...
.feR.mean <- function(x, x.name, decimals = 2, p.sig = 0.05, p.sig.small = 0.01, p.sig.very.small = 0.001) {
  ...
  if (!is.numeric(x)) {
    warning(paste0("[.feR.mean] ERROR - Variable ",x.name," is not numeric"), call. = FALSE)
    return(NA)
  }
